document.getElementById('btnTraerMensajes').addEventListener('click', () => {
    ...
    fetch('https://tpfcac2023.pythonanywhere.com/reservas')
      ...
      .then(datos => {
        ...
        datos.forEach(dato => {
          ...
          fila.innerHTML = `
            ...
            <td>${dato.telefono}</td>
            <td>${dato.mail}</td>
            <td>${dato.cant_comensales}</td>
            <td>${dato.motivoR}</td>
            <td>${dato.primera_vista}</td>
...
document.getElementById('formularioContacto').addEventListener('submit', function(event) {
    ...
    fetch(`https://tpfcac2023.pythonanywhere.com/reservas${id}`, {
      method: 'PUT',
      body: formData
    })
    .then(response => response.json())
